"""Unit-Tests für das GameMode Drohnensteuerungs-Modul (ai_drone.game_actor)."""
...
def test_movement_methods_execution() -> None:
    ...
    # move_backward
    actor.move_backward(duration_s=0.05, speed=0.4)
    assert actor._target_pitch == PWM_NEUTRAL

    # move_right
    actor.move_right(duration_s=0.05, speed=0.4)
    assert actor._target_roll == PWM_NEUTRAL

    # rotate_yaw
    actor.rotate_yaw(duration_s=0.05, speed=0.4)
    assert actor._target_yaw == PWM_NEUTRAL
...
def test_emergency_kill_resets_state() -> None:
    """Testet emergency_kill zum direkten Disarmen und Drosseln."""
    actor, mock_conn = create_test_actor()
    actor.is_flying = True
    actor.is_armed = True

    actor.emergency_kill(reason="Manueller Not-Aus")

    assert actor.is_flying is False
    assert actor.is_armed is False
    assert actor._target_throttle == PWM_THROTTLE_DISARM
    mock_conn.arducopter_disarm.assert_called()
